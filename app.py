...
from db import (
    ...
    get_progress_summary, calculate_streak
)

st.set_page_config(page_title="CGPSC Mains Tracker", page_icon="🎯", layout="wide")

st.markdown("<style>.main-header {font-size: 2.2rem; font-weight: 700; color: #1E3A5F;}</style>", unsafe_allow_html=True)

init_db()
seed_initial_data()

st.sidebar.title("🎯 CGPSC Mains Tracker")
nav = st.sidebar.radio("Navigate", ["Dashboard", "Syllabus Tracker", "Daily Study Log", "Mock Tests", "Analytics", "Settings"])
...
if nav == "Dashboard":
    ...
    col1.metric("Overall Progress", f"{progress['overall_progress']}%")
...
if nav == "Analytics":
    st.info("Full analytics will be added in next version. Your data is being tracked in the database.")

if nav == "Settings":
    st.warning("This is v0.1. More features coming soon!")