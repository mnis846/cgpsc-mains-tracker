"""Study activity logbook — year-long local SQLite journal."""

import pandas as pd

from database import DatabaseError, db_connection
...
def add_activity_log(log_date, activity, subject="", duration_hours=None):
    ...
    with db_connection() as conn:
        c = conn.cursor()
        c.execute(
            """INSERT INTO study_activity_logs (log_date, subject, activity, duration_hours)
               VALUES (?, ?, ?, ?)""",
            (date_str, (subject or "").strip(), activity, duration),
        )
        return c.lastrowid
...
def get_activity_log_stats(year=None):
    ...
    params = []
    if year is not None:
        year_filter = "WHERE strftime('%Y', log_date) = ?"
        params = [str(int(year))]
    with db_connection(commit=False) as conn:
        c = conn.cursor()
        c.execute(f"SELECT COUNT(*) FROM study_activity_logs {year_filter}", params)
        total_entries = int(c.fetchone()[0])
        c.execute(
            f"SELECT COUNT(DISTINCT log_date) FROM study_activity_logs {year_filter}",
            params,
        )
...
def update_activity_log(
    ...
    *,
    ...
    with db_connection() as conn:
        c = conn.cursor()
        c.execute(
            f"UPDATE study_activity_logs SET {', '.join(fields)} WHERE id = ?",
            params,
        )
        if c.rowcount == 0:
...
def delete_activity_log(entry_id):
    ...
    with db_connection() as conn:
        c = conn.cursor()
        c.execute("DELETE FROM study_activity_logs WHERE id = ?", (int(entry_id),))
        if c.rowcount == 0:
            raise DatabaseError("Activity log entry not found.")
