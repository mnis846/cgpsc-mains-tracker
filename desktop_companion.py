...
Death Star, Jupiter, and Saturn in 50-year ULTIMATE forms: hex shields,
...
Two play styles for the desktop coach:

- Normal: calm sticker. Click pokes a short hop that settles (classic).
- Table tennis: cursor is a paddle — ball ricochets off screen edges.

Toggle from the tray menu (or middle-click the coach). Widget stays topmost
and does not steal keyboard focus while you play.

Power saver (ON by default): drops idle redraw to ~4–10 FPS so the sticker
barely uses CPU; hover / bounce / table tennis still run at full ~30 FPS.
"""
...
CAPTION_MS = 14000
REACTION_FRAMES = 36
# Animation cadence — full rate only when the sticker is active.
ANIMATION_MS = 33              # ~30 FPS: bounce, hover, play, reactions
ANIMATION_MS_SAVER = 100       # ~10 FPS: power-saver idle
ANIMATION_MS_DEEP_IDLE = 250   # ~4 FPS: calm sticker, no FX
TIMER_STRIP_H = 28
TRANSPARENT = "#010203"
...
# Shared
BOUNCE_FLOOR_MARGIN = 48  # above taskbar-ish bottom

# Normal mode — gentle poke hop that settles (classic desktop sticker)
NORMAL_GRAVITY = 0.92
NORMAL_RESTITUTION = 0.72
NORMAL_FRICTION = 0.88
NORMAL_AIR_DRAG = 0.995
NORMAL_STOP = 1.15
NORMAL_IMPULSE_Y = (-24.0, -16.0)
NORMAL_IMPULSE_X = 11.0
NORMAL_MAX_SPEED = 28.0

# Table-tennis play mode — lively wall ricochets (window physics)
PLAY_GRAVITY = 0.22
PLAY_RESTITUTION = 0.96
PLAY_FRICTION = 0.97
PLAY_AIR_DRAG = 0.999
PLAY_STOP = 0.9
PLAY_IMPULSE_Y = (-26.0, -18.0)
PLAY_IMPULSE_X = 14.0
PLAY_MAX_SPEED = 38.0
PADDLE_BASE_IMPULSE = 20.0
PADDLE_CURSOR_GAIN = 0.65
PADDLE_HIT_COOLDOWN = 0.16     # seconds between cursor "paddle" hits
PADDLE_MIN_CURSOR_SPEED = 2.5  # re-hit while hovering only if cursor is moving
...
def _hwnd_for(root: tk.Tk) -> int:
    """Resolve the real Win32 HWND for a Tk toplevel."""
...
def _apply_no_activate(root: tk.Tk) -> None:
    """Keep the sticker clickable but never steal keyboard focus from your work."""
    if sys.platform != "win32":
        return
    try:
        import ctypes
...
def _set_topmost_no_activate(root: tk.Tk) -> None:
    """Stay above other windows without activating / stealing the screen."""
    if sys.platform != "win32":
        try:
...
class StudySticker:
    def __init__(self):
        ...
        self._clock_str = ""
        # Bounce physics — starts in calm Normal mode (toggle table tennis from tray)
        self._play_mode = False
        self._bouncing = False
        self._bounce_vx = 0.0
        self._bounce_vy = 0.0
        self._was_hovering = False
        self._last_paddle_hit = 0.0
        self._cursor_screen_x = 0.0
        self._cursor_screen_y = 0.0
        self._cursor_vx = 0.0
        self._cursor_vy = 0.0
        self._cursor_inited = False
        # Power saver ON by default — cuts idle CPU on low-RAM machines (~8 GB).
        # Full ~30 FPS returns automatically on hover / bounce / table tennis.
    ...
    def _refresh_hover_state(self) -> None:
        ...
        try:
            px = float(self.root.winfo_pointerx())
            py = float(self.root.winfo_pointery())
            # Cursor velocity (screen space) — used as paddle swing strength.
            if not self._cursor_inited:
                self._cursor_vx = 0.0
                self._cursor_vy = 0.0
                self._cursor_inited = True
            else:
                self._cursor_vx = px - self._cursor_screen_x
                self._cursor_vy = py - self._cursor_screen_y
            self._cursor_screen_x = px
            self._cursor_screen_y = py
            ...
            if not (wx <= px < wx + ww and wy <= py < wy + wh):
                self._hovering = False
                return
            self._hover_x = int(px - wx)
            self._hover_y = int(py - wy)
    ...
    def _animation_busy(self) -> bool:
        """True when the sticker should run at full frame rate."""
        return bool(
            self._play_mode
            or self._bouncing
            or self._reacting
            ...
            or self._hovering
            or self._particles
            or abs(self._bounce_vx) > 0.01
            or abs(self._bounce_vy) > 0.01
        )

    def _animation_interval_ms(self) -> int:
        """Pick frame delay: full speed when active, throttled when power-saver idle."""
        if not self._power_saver or self._animation_busy():
            return ANIMATION_MS
        if self._idle_kind or self.caption_open:
            return ANIMATION_MS_SAVER
        return ANIMATION_MS_DEEP_IDLE

    def _schedule_animation(self):
        self.root.after(self._animation_interval_ms(), self._on_animation_frame)

    def _set_power_saver(self, enabled: bool, *, notify: bool = True) -> None:
        enabled = bool(enabled)
        if self._power_saver == enabled:
            return
        self._power_saver = enabled
        if notify:
            if enabled:
                _notify(
                    "Study Coach",
                    "Power saver ON — low CPU when idle. Full speed on hover/play.",
                )
            else:
                _notify("Study Coach", "Power saver OFF — smooth ~30 FPS animation.")

    def _toggle_power_saver(self) -> None:
        self._set_power_saver(not self._power_saver)
    ...
    def _on_animation_frame(self):
        try:
            interval_ms = self._animation_interval_ms()
            # Keep motion speed roughly consistent when power-saver slows the frame rate.
            dt_scale = interval_ms / ANIMATION_MS
            self._frame += 1
            self._hull_spin += HULL_SPIN * min(dt_scale, 4.0)
            self._refresh_hover_state()
            self._maybe_paddle_from_cursor()
            self._update_look()
            if self._reacting:
                self._reaction_t += 1
            ...
            if self._idle_kind:
                self._idle_t += 1
                if self._idle_t >= IDLE_FRAMES or self._reacting or self._dragging or self._bouncing:
                    self._idle_t = 0
                ...
                and not self._bouncing
                and not self._hovering
                and self._frame >= self._next_idle_frame
            ):
                self._start_idle()

            if not self._dragging and not self._bouncing:
                p = _profile(self.key)
                pace = max(0.55, p["speed"] * 2.8) * min(dt_scale, 4.0)
                if self._hovering:
                    pace *= 0.55
                ...
                if abs(self._wander_x) >= WANDER_LIMIT:
                    ...
                    self._wander_dir *= -1
            if self._bouncing or abs(self._bounce_vx) > 0.01 or abs(self._bounce_vy) > 0.01:
                self._update_bounce()
            self._draw()
        finally:
            self._schedule_animation()
    ...
    def _keep_topmost(self):
        # Re-assert topmost + no-activate — lift() would steal keyboard focus.
        _apply_no_activate(self.root)
        _set_topmost_no_activate(self.root)
    ...
    def _set_play_mode(self, enabled: bool, *, notify: bool = True) -> None:
        """Enable table-tennis mode, or return to calm Normal mode (and stop the ball)."""
        enabled = bool(enabled)
        if self._play_mode == enabled:
            if not enabled:
                self._stop_bounce()
            return
        self._play_mode = enabled
        if not enabled:
            self._stop_bounce()
            self._was_hovering = False
        if notify:
            if enabled:
                _notify(
                    "Study Coach",
                    "Table tennis ON — touch with cursor to rally. Middle-click to turn off.",
                )
            else:
                _notify("Study Coach", "Normal mode — calm sticker. Click pokes a short hop.")

    def _toggle_play_mode(self) -> None:
        self._set_play_mode(not self._play_mode)

    def _on_middle_click(self, _event) -> None:
        """Quick toggle between Normal and table-tennis without opening the tray."""
        self._toggle_play_mode()

    def _physics(self) -> dict:
        """Active bounce constants for the current mode."""
        if self._play_mode:
            return {
                "gravity": PLAY_GRAVITY,
                "restitution": PLAY_RESTITUTION,
                "friction": PLAY_FRICTION,
                "air_drag": PLAY_AIR_DRAG,
                "stop": PLAY_STOP,
                "impulse_y": PLAY_IMPULSE_Y,
                "impulse_x": PLAY_IMPULSE_X,
                "max_speed": PLAY_MAX_SPEED,
                "wall_kick": True,
            }
        return {
            ...
            "air_drag": NORMAL_AIR_DRAG,
            "stop": NORMAL_STOP,
            "impulse_y": NORMAL_IMPULSE_Y,
            "impulse_x": NORMAL_IMPULSE_X,
            "max_speed": NORMAL_MAX_SPEED,
            "wall_kick": False,
        }

    def _clamp_bounce_speed(self) -> None:
        max_speed = self._physics()["max_speed"]
        speed = math.hypot(self._bounce_vx, self._bounce_vy)
        if speed > max_speed and speed > 0:
            scale = max_speed / speed
            self._bounce_vx *= scale
            self._bounce_vy *= scale

    def _shrink_for_play(self) -> None:
        """Drop caption balloon while playing so the window is a tighter 'ball'."""
        if not self.caption_open:
            return
        self.caption_open = False
        self._resize()

    def _maybe_paddle_from_cursor(self) -> None:
        """Cursor paddle only in table-tennis mode."""
        if not self._play_mode or self._dragging:
            self._was_hovering = self._hovering
            return

        just_touched = self._hovering and not self._was_hovering
        cursor_speed = math.hypot(self._cursor_vx, self._cursor_vy)
        swinging = (
            self._hovering
            and self._was_hovering
            and cursor_speed >= PADDLE_MIN_CURSOR_SPEED
        )
        self._was_hovering = self._hovering

        if not just_touched and not swinging:
            return
        if time.time() - self._last_paddle_hit < PADDLE_HIT_COOLDOWN:
            return
        self._paddle_hit(self._hover_x, self._hover_y)

    def _paddle_hit(self, px: int, py: int) -> None:
        """Table-tennis paddle strike — ball flies away from the cursor."""
        _, _, sw, sh, ox, oy = self._char_base()
        cx = ox + sw / 2
        cy = oy + sh / 3
        dx = cx - px
        ...
        # Prefer outward normal; if cursor is nearly on center, use cursor swing.
        if dist < 10:
            swing = math.hypot(self._cursor_vx, self._cursor_vy)
            if swing > 0.5:
                nx = self._cursor_vx / swing
                ny = self._cursor_vy / swing
            else:
                nx = random.choice((-1.0, 1.0))
                ny = random.uniform(-0.85, -0.25)

        # Hit offset adds side-spin (classic paddle feel).
        hit_offset = (px - cx) / max(1.0, sw / 2)
        hit_offset = max(-1.2, min(1.2, hit_offset))

        impulse = PADDLE_BASE_IMPULSE + random.uniform(2.0, 7.0)
        self._bounce_vx = (
            nx * impulse
            + self._cursor_vx * PADDLE_CURSOR_GAIN
            - hit_offset * PLAY_IMPULSE_X * 0.55
            + random.uniform(-2.0, 2.0)
        )
        self._bounce_vy = (
            ny * impulse
            + self._cursor_vy * PADDLE_CURSOR_GAIN
            + random.uniform(*PLAY_IMPULSE_Y) * 0.35
        )
        # Prefer a lively launch upward on soft taps.
        if abs(self._bounce_vy) < 10:
            self._bounce_vy = random.uniform(*PLAY_IMPULSE_Y)

        self._clamp_bounce_speed()
        self._bouncing = True
        self._last_paddle_hit = time.time()
        if abs(self._bounce_vx) > 0.4:
            self._facing = 1 if self._bounce_vx > 0 else -1

        self._shrink_for_play()
        # Light react juice — no caption so it doesn't eat the screen.
        self._reacting = True
        self._reaction_t = 0
        self._idle_kind = None
        self._charge_level = 1.0
        self._spawn_particles(16, px, py)

    def _poke_bounce(self, px: int, py: int) -> None:
        """Click hit — paddle serve in play mode, gentle hop in normal mode."""
        if self._play_mode:
            self._paddle_hit(px, py)
            return

        # Classic calm hop: settles after a few wall/floor bounces.
        _, _, sw, _sh, ox, _oy = self._char_base()
        cx = ox + sw / 2
        hit_offset = (px - cx) / max(1.0, sw / 2)
        hit_offset = max(-1.2, min(1.2, hit_offset))
        self._bounce_vx += hit_offset * NORMAL_IMPULSE_X + random.uniform(-2.5, 2.5)
        kick = random.uniform(*NORMAL_IMPULSE_Y)
        self._bounce_vy = min(0.0, self._bounce_vy) + kick
        self._clamp_bounce_speed()
        self._bouncing = True
        if abs(self._bounce_vx) > 0.4:
            self._facing = 1 if self._bounce_vx > 0 else -1
        ...
        self._idle_kind = None
        self._charge_level = 1.0
        self._spawn_particles(14, px, py)

    def _update_bounce(self) -> None:
        """Gravity + wall bounce. Play mode ricochets hard; normal mode settles."""
        if self._dragging:
            self._stop_bounce()
            return

        phys = self._physics()
        self._bounce_vy += phys["gravity"]
        self._bounce_vx *= phys["air_drag"]
        self._bounce_vy *= phys["air_drag"]

        try:
            x = float(self.root.winfo_x()) + self._bounce_vx
            y = float(self.root.winfo_y()) + self._bounce_vy
        ...
        left, top = 4.0, 4.0
        right = float(sw - w - 4)
        floor = float(sh - h - BOUNCE_FLOOR_MARGIN)
        if floor < top + 40:
            floor = float(sh - h - 8)

        rest = phys["restitution"]
        fric = phys["friction"]
        stop = phys["stop"]
        kick = phys["wall_kick"]
        hit = False
        ...
        if y >= floor:
            y = floor
            if self._bounce_vy > 0:
                self._bounce_vy *= -rest
                self._bounce_vx *= fric
                if kick:
                    self._bounce_vx += random.uniform(-0.6, 0.6)
                hit = True
                if abs(self._bounce_vy) < stop:
                    self._bounce_vy = 0.0
        # Ceiling
        elif y < top:
            y = top
            if self._bounce_vy < 0:
                self._bounce_vy *= -rest
                if kick:
                    self._bounce_vx += random.uniform(-0.5, 0.5)
                hit = True

        # Side walls
        if x < left:
            x = left
            if self._bounce_vx < 0:
                self._bounce_vx *= -rest
                if kick:
                    self._bounce_vy += random.uniform(-1.2, 1.2)
                hit = True
        elif x > right:
            x = right
            if self._bounce_vx > 0:
        ...
        particle_threshold = 2.0 if self._play_mode else 2.5
        if hit and (abs(self._bounce_vy) > particle_threshold or abs(self._bounce_vx) > particle_threshold):
            self._spawn_particles(4 if self._play_mode else 5)
            if abs(self._bounce_vx) > 0.5:
                self._facing = 1 if self._bounce_vx > 0 else -1

        # Settled on the floor with almost no energy left.
        if (
            abs(self._bounce_vx) < stop
            and abs(self._bounce_vy) < stop
            and y >= floor - 1.5
        ):
            self._stop_bounce()
            y = floor

        self.root.geometry(f"{w}x{h}+{int(round(x))}+{int(round(y))}")
    ...
    def _trigger_reaction(self, px: int | None = None, py: int | None = None, *, with_caption: bool = True):
    ...
    def _on_press(self, event):
        ...
        if self._hit_character(event.x, event.y):
            # Playful bounce toy — no caption balloon, no focus steal.
            self._poke_bounce(event.x, event.y)
...
def _snap_sticker(sticker: StudySticker):
    ...
    sticker._wander_x = 0
    sticker._stop_bounce()
    sticker._place_corner()
    sticker._resize()
    sticker._draw()


def _tray_call(sticker: StudySticker, fn) -> None:
    """Run sticker UI work on the Tk thread (tray callbacks are off-thread)."""
    try:
        sticker.root.after(0, fn)
    except Exception:
        fn()
...
def _run_tray(sticker: StudySticker):
    ...
    menu = pystray.Menu(
        ...
        pystray.MenuItem("Next coach", lambda *_: _tray_call(sticker, lambda: sticker._on_right_click(None))),
        pystray.MenuItem("Snap top-right", lambda *_: _tray_call(sticker, lambda: _snap_sticker(sticker))),
        pystray.Menu.SEPARATOR,
        pystray.MenuItem(
            "Power saver",
            lambda *_: _tray_call(sticker, sticker._toggle_power_saver),
            checked=lambda _item: sticker._power_saver,
        ),
        pystray.MenuItem(
            "Table tennis mode",
            lambda *_: _tray_call(sticker, sticker._toggle_play_mode),
            checked=lambda _item: sticker._play_mode,
        ),
        pystray.MenuItem(
            "Stop bounce",
            lambda *_: _tray_call(sticker, sticker._stop_bounce),
        ),
        pystray.Menu.SEPARATOR,
