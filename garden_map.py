"""Panoramic study path — 55 prelims trees + mains sprint, drag to explore."""
...
def render_garden_world(garden_state, height=780):
    ...
    <div class="zen-wrap">
      ...
      <div class="zen-hint">Drag the path · 1 tree / 4 study days · {PRELIMS_TREE_TARGET} prelims · {MAX_GROVE_TREES} total · &gt;60% = sakura</div>
    </div>
    <style>
      .zen-wrap {{
    ...
    (function() {{
      ...
      function drawWorldBg() {{
        ...
        for (let row = 0; row < rows; row++) {{
          const isMains = row >= Math.floor(D.prelimsTarget / PER_ROW);
          const y = 440 + row * ROW_H;
          const grd = ctx.createLinearGradient(0, y, 0, y + ROW_H + 40);
          grd.addColorStop(0, isMains ? '#2e4a32' : '#2a4530');
          grd.addColorStop(1, isMains ? '#1a3020' : '#152818');
          ctx.fillStyle = grd;
      ...
      function drawMinimap() {{
        ...
        const gateY = Math.floor(D.prelimsTarget / PER_ROW) * ROW_H;
        ...
        ctx.fillText('PRELIMS', mx+6, my+10);
        ctx.fillText('MAINS', mx+6, my+mh-6);
      }}
      ...
      function drawHud() {{
        ...
        ctx.fillText(
          D.treeCount + '/' + D.maxTrees + ' trees · Prelims ' + D.prelimsTrees + '/' + D.prelimsTarget +
        ...
        const pw = bw * (D.prelimsTrees / D.prelimsTarget);
        ...
        ctx.fillText('Prelims grove progress', 30, 114);
      ...
      function frame() {{
        ...
        drawRiver();
        drawPath();
        drawMilestoneGate(D.prelimsTarget, 'PRELIMS EVE · TREE ' + D.prelimsTarget, '#ff8f00');
        drawMilestoneGate(D.prelimsTarget + 1, 'MAINS SPRINT', '#ce93d8');
        D.trees.forEach(tree => {{
          const wp = treeWorldPos(tree.slot);
          const sp = screenPos(wp);
