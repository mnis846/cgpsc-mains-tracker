"""
Death Star desktop watcher — ULTIMATE evolution form.
...
Launch: python deathstar_watcher.py  (or Start Death Star Watcher.bat)
"""
...
CHARACTER = "deathstar"
MUTEX_NAME = "Global\\CGPSC.DeathStarWatcher"
...
def _notify(title: str, message: str) -> None:
    try:
        ...
        notification.notify(title=title, message=message, app_name="Death Star", timeout=10)
    except Exception:
        pass
...
def _quick_log_hours(hours: float) -> None:
    ...
    try:
        ...
        init_db()
        add_daily_study_hours(date.today(), hours, "Quick log - Death Star")
        try:
...
def _run_tray(watcher: DeathStarWatcher):
    ...
    menu = pystray.Menu(
        ...
        pystray.MenuItem("Quit Death Star", lambda icon, *_: watcher._request_shutdown(icon)),
    )
    icon = pystray.Icon("death_star", _tray_image(), "Death Star — right-click to quit", menu)
